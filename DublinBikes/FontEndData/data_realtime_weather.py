...
def save_weather_data_to_cache_db(data: dict, forecast_type: str, target_datetime: datetime.datetime, return_row: bool) -> None:
    """
    ...
    try:
        ...
        if return_row:
            ...
            cursor.execute("SELECT * FROM FetchedWeatherData WHERE rowid = ?", (inserted_rowid,))
            row = cursor.fetchone()
...
def get_current_weather_data():
    ...
    if row:    
        ...
        if data_text:
            ...
            return save_weather_data_to_cache_db(current, "current", dt_current, return_row = True)                
...
def get_forecast_weather_data(forecast_type: str, target_datetime: str) -> dict:
    ...
    # Else, ask the API, and save cache data from the API for all the forecasts given
        ...
        for forecast in forecast_list:
            ...
            if abs((entry_dt - target_dt).total_seconds()) < 5470:  # 3 hours = 5400 seconds, we put a bit more just in case
                matching_forecast = save_weather_data_to_cache_db(forecast, forecast_type, target_datetime, return_row=True)
            
            else:
                save_weather_data_to_cache_db(forecast, forecast_type, target_datetime, return_row=False)
