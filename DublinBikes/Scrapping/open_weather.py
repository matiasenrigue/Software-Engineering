import requests
import traceback
import datetime
import time
import os
import json
import sqlalchemy as sqla


from DublinBikes.Utils.params import WEATHER_KEY
from DublinBikes.Utils.sql_utils import get_sql_engine, execute_sql



# OpenWeatherMap API endpoint (current weather)
CURRENT_OPENWEATHER_URI = "http://api.openweathermap.org/data/2.5/weather"


def get_data_from_openweather() -> str:
    """
    Function to get the weather data from the OpenWeatherMap API for Dublin.
    
    :return: the data from the API (text)
    """
    try:
        # Hardcoded parameters for Dublin, Ireland
        params = {
            "q": "Dublin,IE",
            "appid": WEATHER_KEY,
            "units": "metric",
            "lang": "en"
        }
        r = requests.get(CURRENT_OPENWEATHER_URI, params=params)
        r.raise_for_status()
        return r.text
    except:
        print(traceback.format_exc())
        return None



def save_data_to_db(data: str, in_engine: sqla.engine.base.Connection) -> None:
    """
    Function to load the weather data into the database.
    
    :param data: the data to be loaded (text)
    :param in_engine: the engine to be used to load the data
    """
    print(data)
    
    save_current_data_to_db(data, in_engine) # Done 
    # save_daily_data_to_db(data, in_engine)   --> I'm not sure which API to use
    # save_hourly_data_to_db(data, in_engine)  --> I'm not sure which API to use



def save_current_data_to_db(data: str, in_engine: sqla.engine.base.Connection) -> None:
    """
    Function to load the current weather data into the database.

    Table structure (current):
        dt DATETIME NOT NULL,
        feels_like FLOAT,
        humidity INTEGER,
        pressure INTEGER,
        sunrise DATETIME,
        sunset DATETIME,
        temp FLOAT,
        uvi FLOAT,
        weather_id INTEGER,
        wind_gust FLOAT,
        wind_speed FLOAT,
        rain_1h FLOAT,
        snow_1h FLOAT,
        PRIMARY KEY (dt)
    """
    try:
        current = json.loads(data)
        
        dt_current = datetime.datetime.fromtimestamp(current.get("dt"))
        weather_id = current.get("id") if "id" in current else 'NULL'

        main = current.get("main", {})
        feels_like = main.get("feels_like") if "feels_like" in main else 'NULL'
        humidity = main.get("humidity") if "humidity" in main else 'NULL'
        pressure = main.get("pressure") if "pressure" in main else 'NULL'
        temp = main.get("temp") if "temp" in main else 'NULL'
        
        sys = current.get("sys", {})
        sunrise = datetime.datetime.fromtimestamp(sys.get("sunrise")) if "sunrise" in sys else 'NULL'
        sunset = datetime.datetime.fromtimestamp(sys.get("sunset")) if "sunset" in sys else 'NULL'
        
        wind = current.get("wind", {})
        wind_speed = wind.get("speed") if "speed" in wind else 'NULL'
        wind_gust = wind.get("gust") if "gust" in wind else 'NULL'
        
        rain = current.get("rain", {})
        rain_1h = rain.get("1h") if "1h" in rain else 'NULL'
        
        uvi = 'NULL'     # No info 
        snow_1h = 'NULL' # No info

        query = f"""
            INSERT INTO current 
            (dt, feels_like, humidity, pressure, sunrise, sunset, temp, uvi, weather_id, wind_gust, wind_speed, rain_1h, snow_1h)
            VALUES (
                '{dt_current}', 
                {feels_like}, 
                {humidity}, 
                {pressure}, 
                '{sunrise}', 
                '{sunset}', 
                {temp}, 
                {uvi}, 
                {weather_id}, 
                {wind_gust}, 
                {wind_speed}, 
                {rain_1h}, 
                {snow_1h}
            );
        """
        execute_sql(query, in_engine)
    except Exception as e:
        print("Error saving current weather data:", e)
        print(traceback.format_exc())




def save_hourly_data_to_db(data: str, in_engine: sqla.engine.base.Connection) -> None:
    """
    Function to load the hourly forecast data into the database.

    Table structure (hourly):
        dt DATETIME NOT NULL,
        future_dt DATETIME NOT NULL,
        feels_like FLOAT,
        humidity INTEGER,
        pop FLOAT,
        pressure INTEGER,
        temp FLOAT,
        uvi FLOAT,
        weather_id INTEGER,
        wind_gust FLOAT,
        wind_speed FLOAT,
        rain_1h FLOAT,
        snow_1h FLOAT,
        PRIMARY KEY (dt, future_dt)
    """
    try:
        weather_data = json.loads(data)
        # Use the current time as the retrieval timestamp for all hourly records.
        dt_retrieval = datetime.datetime.now()
        hourly_list = weather_data.get("hourly", [])

        for hour in hourly_list:
            future_dt = datetime.datetime.fromtimestamp(hour.get("dt"))
            feels_like = hour.get("feels_like")
            humidity = hour.get("humidity")
            pop = hour.get("pop")
            pressure = hour.get("pressure")
            temp = hour.get("temp")
            uvi = hour.get("uvi")
            weather_list = hour.get("weather", [])
            weather_id = weather_list[0].get("id") if weather_list else 'NULL'
            wind_speed = hour.get("wind_speed")
            wind_gust = hour.get("wind_gust") if hour.get("wind_gust") is not None else 'NULL'
            rain_1h = hour.get("rain", {}).get("1h") if hour.get("rain") else 'NULL'
            snow_1h = hour.get("snow", {}).get("1h") if hour.get("snow") else 'NULL'

            query = f"""
                INSERT INTO hourly 
                (dt, future_dt, feels_like, humidity, pop, pressure, temp, uvi, weather_id, wind_gust, wind_speed, rain_1h, snow_1h)
                VALUES (
                    '{dt_retrieval}', 
                    '{future_dt}', 
                    {feels_like}, 
                    {humidity}, 
                    {pop}, 
                    {pressure}, 
                    {temp}, 
                    {uvi}, 
                    {weather_id}, 
                    {wind_gust}, 
                    {wind_speed}, 
                    {rain_1h}, 
                    {snow_1h}
                );
            """
            execute_sql(query, in_engine)
    except Exception as e:
        print("Error saving hourly forecast data:", e)
        print(traceback.format_exc())




def save_daily_data_to_db(data: str, in_engine: sqla.engine.base.Connection) -> None:
    """
    Function to load the daily forecast data into the database.

    Table structure (daily):
        dt DATETIME NOT NULL,
        future_dt DATETIME NOT NULL,
        humidity INTEGER,
        pop FLOAT,
        pressure INTEGER,
        temp_max FLOAT,
        temp_min FLOAT,
        uvi FLOAT,
        weather_id INTEGER,
        wind_speed FLOAT,
        wind_gust FLOAT,
        rain FLOAT,
        snow FLOAT,
        PRIMARY KEY (dt, future_dt)
    """
    try:
        weather_data = json.loads(data)
        # Use the current time as the retrieval timestamp for all daily records.
        dt_retrieval = datetime.datetime.now()
        daily_list = weather_data.get("daily", [])

        for day in daily_list:
            future_dt = datetime.datetime.fromtimestamp(day.get("dt"))
            humidity = day.get("humidity")
            pop = day.get("pop")
            pressure = day.get("pressure")
            temp_max = day.get("temp", {}).get("max")
            temp_min = day.get("temp", {}).get("min")
            uvi = day.get("uvi")
            weather_list = day.get("weather", [])
            weather_id = weather_list[0].get("id") if weather_list else 'NULL'
            wind_speed = day.get("wind_speed")
            wind_gust = day.get("wind_gust") if day.get("wind_gust") is not None else 'NULL'
            rain = day.get("rain") if "rain" in day else 'NULL'
            snow = day.get("snow") if "snow" in day else 'NULL'

            query = f"""
                INSERT INTO daily 
                (dt, future_dt, humidity, pop, pressure, temp_max, temp_min, uvi, weather_id, wind_speed, wind_gust, rain, snow)
                VALUES (
                    '{dt_retrieval}', 
                    '{future_dt}', 
                    {humidity}, 
                    {pop}, 
                    {pressure}, 
                    {temp_max}, 
                    {temp_min}, 
                    {uvi}, 
                    {weather_id}, 
                    {wind_speed}, 
                    {wind_gust}, 
                    {rain}, 
                    {snow}
                );
            """
            execute_sql(query, in_engine)
    except Exception as e:
        print("Error saving daily forecast data:", e)
        print(traceback.format_exc())






def main_data_scrapper_weather():
    """
    Main function for the weather data scrapper.
    """
    engine = get_sql_engine()
    
    while True:
        weather_data = get_data_from_openweather()
        
        if weather_data:
            save_data_to_db(weather_data, engine)

        else:
            print("No Weather Data")
        
        time.sleep(5 * 60)



if __name__ == "__main__":
    main_data_scrapper_weather()
